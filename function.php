...
function hapusGejala($id_gejala)
{
    ...
    // Hapus terlebih dahulu data terkait dari tabel relasi
    mysqli_query($koneksi, "DELETE FROM relasi WHERE id_gejala = $id_gejala");

    // Cek jumlah baris yang terpengaruh oleh penghapusan relasi
    $resultRelasi = mysqli_affected_rows($koneksi);

    if ($resultRelasi > 0) {
        // Jika ada data terkait di tabel relasi, lanjutkan untuk menghapus gejala
        mysqli_query($koneksi, "DELETE FROM gejala WHERE id_gejala = $id_gejala");
        $resultGejala = mysqli_affected_rows($koneksi);
        ...
        // Jika tidak ada data terkait di tabel relasi
        echo "
        <script>
            alert('Gejala tidak dapat dihapus karena masih terkait dengan penyakit!');
            document.location.href = 'indexGejala.php';
        </script>";
    }
}
